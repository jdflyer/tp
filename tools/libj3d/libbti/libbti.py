...
def encodeRGB565(color):
    return (((color[0]//8)&0b11111) << 11) | (((color[1]//4)&0b111111) << 5) | ((color[2]//8)&0b11111)

def decodeRGB5A3(bytes):
    full = (bytes[0]<<8) | bytes[1]
    isAlpha = not (full & 0x8000) > 0
    if isAlpha:
        return ((full>>8)&0xF)*0x11,((full>>4)&0xF)*0x11,(full&0xF)*0x11,((full>>12)&0b111)*0x20
    else:
        return ((full>>10)&0b11111)*8,((full>>5)&0b11111)*8,(full&0b11111)*8,0xff

def encodeRGB5A3(color):
    isAlpha = color[3] < 0xff
    if isAlpha:
        return (((color[3]//0x20)&0b111) << 12) | (((color[0]//0x11)&0xf) << 8) | (((color[1]//0x11)&0xf) << 4) | ((color[2]//0x11)&0xf)
    else:
        return (1<<15) | (((color[0]//8)&0b11111) << 10) | (((color[1]//8)&0b11111) << 5) | ((color[2]//8)&0b11111)
...
class ImageBase:
    def __init__(self,header,data = None,imageBuffer = None):
        ...
        if self.data == None:
            self.data = bytearray()
        self.imageBuffer = imageBuffer if imageBuffer != None else [(0,0,0,0xff)]*(header.width*header.height)
    ...
    def decodeLoop(self,blockWidth,blockHeight,blockStride):
        ...
        for blockY in range(heightInBlocks):
            for blockX in range(widthInBlocks):
                self.decodeBlock(blockX*blockWidth,blockY*blockHeight,self.data[offset:offset+blockStride])
                offset += blockStride
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for inY in range(4):
            for inX in range(8):
                ...
                self.writePixel(x+inX,y+inY,(intensity, intensity, intensity, 0xff))
                offset += 1
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for inY in range(4):
            for inX in range(8):
                ...
                self.writePixel(x+inX,y+inY,(intensity, intensity, intensity, (byte>>4)*0x11))
                offset += 1
...
class IA8Image(ImageBase):
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for inY in range(4):
            for inX in range(4):
                self.writePixel(x+inX,y+inY,decodeIA8(blockBytes[offset:offset+2]))
                offset += 2
...
class RGB565Image(ImageBase):
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for inY in range(4):
            for inX in range(4):
                self.writePixel(x+inX,y+inY,decodeRGB565(blockBytes[offset:offset+2]))
                offset += 2
...
class RGB5A3Image(ImageBase):
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for inY in range(4):
            for inX in range(4):
                self.writePixel(x+inX,y+inY,decodeRGB5A3(blockBytes[offset:offset+2]))
                offset += 2
...
class RGBA32Image(ImageBase):
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for inY in range(4):
            for inX in range(4):
                self.writePixel(x+inX,y+inY,(blockBytes[rOffset],blockBytes[gOffset],blockBytes[bOffset],blockBytes[aOffset]))
                aOffset += 2
                rOffset += 2
                gOffset += 2
                bOffset += 2
...
class C4Image(ImageBase):
    ...
    def encode(self):
        ...
        assert self.header.mipmapEnabled == 0 and self.header.mipmapCount == 1, "Mipmaps are not supported on Paletted textures!"
        self.encodeLoop(8,8)
        self.header.numColors = len(self.paletteColors)
        self.header.paletteOffset = len(self.data) + 0x20
        self.encodePaletteColors()
        ...
        for inY in range(4):
            for inX in range(8):
                ...
                if blockBytes[offset] >= self.header.numColors:
                    offset += 1
                    continue # Out of bounds, likely due to the block going out of the image
                self.writePixel(x+inX,y+inY,self.paletteColors[blockBytes[offset]])
                offset += 1
...
class CMPRImage(ImageBase):
    ...
    def decodeBlock(self,x,y,blockBytes):
        ...
        for subY in range(2):
            for subX in range(2):
                colors = [decodeRGB565(blockBytes[offset+0:offset+2]),decodeRGB565(blockBytes[offset+2:offset+4])]
                realVal1, realVal2 = struct.unpack(">HH",blockBytes[offset+0:offset+4])
                ...
                for inY in range(4):
                    for inX in range(4):
                        self.writePixel(x+(subX*4)+inX,y+(subY*4)+inY,colors[val>>30])
    ...
    def encodeBlock(self,x,y):
        for subY in range(2):
            for subX in range(2):
                block = [self.readPixel(x+(subX*4)+inX,y+(subY*4)+inY) for inY in range(4) for inX in range(4)]
                # print(f"{x} {y}")
                # print(block)
                ...
                for color in block:
                    ...
                    distances = [self.color_distance(color, testColor) for testColor in colors]
                    indices.append(distances.index(min(distances)))
                
                indicesInt = 0
                for i in range(16):
                    indicesInt = (indicesInt << 2) | (indices[i]&0b11) # (indicesInt >> 2) | ((indices[i]&0b11)<<30)
...
def namedValueFromDict(value,dict):
    if not value in dict:
        print(f"Format {value} is not a valid type. Valid types are {", ".join(dict.keys())}")
        return None
    return dict[value]
        ...
        for i in range(header.mipmapCount):
            ...
            for y in range(header.height):
                for x in range(header.width):
                    ...
                    p3 = image.imageBuffer[(header.width*2)+(y*header.width*4)+(x*2)]
                    p4 = image.imageBuffer[(header.width*2)+(y*header.width*4)+(x*2)+1]
...
def main():
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} input.bti output.png OR {sys.argv[0]} input.png output.bti")
        exit(1)
